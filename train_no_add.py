...
parse = parser_train()
parse.add_argument('--tau', type=float, default=0.9975, help='Weight averaging decay.')    # sqrt of 0.995
args = parse.parse_args()
# assert args.data in SEMISUP_DATASETS, f'Only data in {SEMISUP_DATASETS} is supported!'    # seems like cifar10 is illegal, cifar10s is good
...
seed(args.seed)
train_dataset, test_dataset, train_dataloader, test_dataloader = load_data(    # main modification
    DATA_DIR, BATCH_SIZE, BATCH_SIZE_VALIDATION, use_augmentation=args.augment, shuffle_train=True, 
    aux_data_filename=None, unsup_fraction=None, validation=False    # the code of core/data/__init__ seems like the validation should be False if we don't use additional data
)
